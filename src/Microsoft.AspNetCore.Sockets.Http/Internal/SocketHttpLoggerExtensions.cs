...
namespace Microsoft.AspNetCore.Sockets.Internal
{
    internal static class SocketHttpLoggerExtensions
    {
        ...
        private static readonly Action<ILogger, Exception> _longPolling204 =
            LoggerMessage.Define(LogLevel.Information, new EventId(0, nameof(LongPolling204)), "Terminating Long Polling connection by sending 204 response.");

        private static readonly Action<ILogger, Exception> _pollTimedOut =
            LoggerMessage.Define(LogLevel.Information, new EventId(1, nameof(PollTimedOut)), "Poll request timed out. Sending 200 response to connection.");

        private static readonly Action<ILogger, int, Exception> _longPollingWritingMessage =
            LoggerMessage.Define<int>(LogLevel.Debug, new EventId(2, nameof(LongPollingWritingMessage)), "Writing a {count} byte message to connection.");

        private static readonly Action<ILogger, Exception> _longPollingDisconnected =
            LoggerMessage.Define(LogLevel.Debug, new EventId(3, nameof(LongPollingDisconnected)), "Client disconnected from Long Polling endpoint for connection.");

        private static readonly Action<ILogger, Exception> _longPollingTerminated =
            LoggerMessage.Define(LogLevel.Error, new EventId(4, nameof(LongPollingTerminated)), "Long Polling transport was terminated due to an error on connection.");

        // Category: HttpConnectionDispatcher
        private static readonly Action<ILogger, string, Exception> _connectionDisposed =
            LoggerMessage.Define<string>(LogLevel.Debug, new EventId(0, nameof(ConnectionDisposed)), "Connection Id {connectionId} was disposed.");

        private static readonly Action<ILogger, string, string, Exception> _connectionAlreadyActive =
            LoggerMessage.Define<string, string>(LogLevel.Debug, new EventId(1, nameof(ConnectionAlreadyActive)), "Connection Id {connectionId} is already active via {requestId}.");

        private static readonly Action<ILogger, string, string, Exception> _pollCanceled =
            LoggerMessage.Define<string, string>(LogLevel.Debug, new EventId(2, nameof(PollCanceled)), "Previous poll canceled for {connectionId} on {requestId}.");

        private static readonly Action<ILogger, Exception> _establishedConnection =
            LoggerMessage.Define(LogLevel.Debug, new EventId(3, nameof(EstablishedConnection)), "Establishing new connection.");

        private static readonly Action<ILogger, Exception> _resumingConnection =
            LoggerMessage.Define(LogLevel.Debug, new EventId(4, nameof(ResumingConnection)), "Resuming existing connection.");

        private static readonly Action<ILogger, int, Exception> _receivedBytes =
            LoggerMessage.Define<int>(LogLevel.Debug, new EventId(5, nameof(ReceivedBytes)), "Received {count} bytes.");

        private static readonly Action<ILogger, TransportType, Exception> _transportNotSupported =
            LoggerMessage.Define<TransportType>(LogLevel.Debug, new EventId(6, nameof(TransportNotSupported)), "{transportType} transport not supported by this endpoint type.");

        private static readonly Action<ILogger, TransportType, TransportType, Exception> _cannotChangeTransport =
            LoggerMessage.Define<TransportType, TransportType>(LogLevel.Debug, new EventId(7, nameof(CannotChangeTransport)), "Cannot change transports mid-connection; currently using {transportType}, requesting {requestedTransport}.");

        private static readonly Action<ILogger, Exception> _postNotallowedForWebsockets =
            LoggerMessage.Define(LogLevel.Debug, new EventId(8, nameof(PostNotAllowedForWebSockets)), "POST requests are not allowed for websocket connections.");

        private static readonly Action<ILogger, Exception> _negotiationRequest =
            LoggerMessage.Define(LogLevel.Debug, new EventId(8, nameof(NegotiationRequest)), "Sending negotiation response.");

        // Category: WebSocketsTransport
        private static readonly Action<ILogger, Exception> _socketOpened =
            LoggerMessage.Define(LogLevel.Information, new EventId(0, nameof(SocketOpened)), "Socket opened.");

        private static readonly Action<ILogger, Exception> _socketClosed =
            LoggerMessage.Define(LogLevel.Information, new EventId(1, nameof(SocketClosed)), "Socket closed.");

        private static readonly Action<ILogger, WebSocketCloseStatus?, string, Exception> _clientClosed =
            LoggerMessage.Define<WebSocketCloseStatus?, string>(LogLevel.Debug, new EventId(2, nameof(ClientClosed)), "Client closed connection with status code '{status}' ({description}). Signaling end-of-input to application..");
        ...
        private static readonly Action<ILogger, Exception> _failedSending =
            LoggerMessage.Define(LogLevel.Debug, new EventId(4, nameof(FailedSending)), "Application failed during sending. Sending InternalServerError close frame.");

        private static readonly Action<ILogger, Exception> _finishedSending =
            LoggerMessage.Define(LogLevel.Debug, new EventId(5, nameof(FinishedSending)), "Application finished sending. Sending close frame.");

        private static readonly Action<ILogger, Exception> _waitingForClose =
            LoggerMessage.Define(LogLevel.Debug, new EventId(6, nameof(WaitingForClose)), "Waiting for the client to close the socket.");

        private static readonly Action<ILogger, Exception> _closeTimedOut =
            LoggerMessage.Define(LogLevel.Debug, new EventId(7, nameof(CloseTimedOut)), "Timed out waiting for client to send the close frame, aborting the connection.");

        private static readonly Action<ILogger, WebSocketMessageType, int, bool, Exception> _messageReceived =
            LoggerMessage.Define<WebSocketMessageType, int, bool>(LogLevel.Debug, new EventId(8, nameof(MessageReceived)), "Message received. Type: {messageType}, size: {size}, EndOfMessage: {endOfMessage}.");

        private static readonly Action<ILogger, int, Exception> _messageToApplication =
            LoggerMessage.Define<int>(LogLevel.Debug, new EventId(9, nameof(MessageToApplication)), "Passing message to application. Payload size: {size}.");

        private static readonly Action<ILogger, int, Exception> _sendPayload =
            LoggerMessage.Define<int>(LogLevel.Debug, new EventId(10, nameof(SendPayload)), "Sending payload: {size} bytes.");

        private static readonly Action<ILogger, Exception> _errorWritingFrame =
            LoggerMessage.Define(LogLevel.Error, new EventId(11, nameof(ErrorWritingFrame)), "Error writing frame.");

        private static readonly Action<ILogger, Exception> _sendFailed =
            LoggerMessage.Define(LogLevel.Trace, new EventId(12, nameof(SendFailed)), "Socket failed to send.");

        // Category: ServerSentEventsTransport
        private static readonly Action<ILogger, int, Exception> _sseWritingMessage =
            LoggerMessage.Define<int>(LogLevel.Debug, new EventId(0, nameof(SSEWritingMessage)), "Writing a {count} byte message.");
